		...
		dform : {
			...
			methods : {
				...
				append : function (options, converter) {
					...
					{	if(options.validate.required==true)
		...
		"caption" : function (options, type) {
			...
			if (type == "fieldset") {
				...
				if ( type === "checkbox" || type === "radio" || type === "radiobutton") {				
						...
						var parent_attributes= new Array();
						parent_attributes.push(this.parent().attr("id"));
						parent_attributes.push(this.parent().attr("required"));
						parent_attributes.push(this.parent().attr("class"));
						var this_attributes=new Array();
						this_attributes.push(this.attr("type"));
						this_attributes.push(this.attr("required"));
						this_attributes.push(this.attr("class"));
						var parent_is_form=is_radio=is_single_radio=this_has_requird=0;
						for(var n in parent_attributes) {
							if(this.parent().attr("method")!==undefined )
							{	parent_is_form=1;
							}							
						}
						for(var n in this_attributes) {
							if(this_attributes[n]=="ui-dform-radiobutton" )
							{	is_single_radio=1;
							}
							else if(this_attributes[n]=="required" )
							{	this_has_requird=1;
							}
						}
						var required_attr=new Array();
						this.each(function() {
						  $.each(this.attributes, function() {
							if(this.specified) {
							  required_attr.push(this.name)
							}
						  });
						});
						var label_required_class='';
						var input_required='';
						if($.inArray("required", required_attr))
						{	label_required_class='dform-label-required';
							input_required='required="required"';
						}
						if(is_single_radio==0)
						{ 	if(parent_is_form==0) //this handles multiple radio and checkboxes
							{	var lab=ops["html"]; 
								var that=this.parent();
								var opt_id=that[0]['id']+"_"+ops["html"].toLowerCase().replace(/ /g, "_").replace(/\W/g, '');
								if(type === "checkbox" )
									{	//console.log(ops["html"].toLowerCase().replace(/ /g, "_").replace(/\W/g, ''));
										this.attr("name", that[0]['id']+'[]');	
										this.attr("id", opt_id);				
									}
									else
									{	this.attr("name", that[0]['id']);			
								...
								this.wrap("<label class='"+type+" control-label'></label>" );				
									
								this.parent().append($(label).children());
								this.parent().append("<span class='option-label'>"+ops["html"]+"</span>")		;
								this.parent().prop("for", opt_id);
								//console.log(this.parent().parent().parent().parent().get(0).tagName);
								/*START - FIELDSET AROUND GROUP*/
								if(this.parent().parent().parent().parent().get(0).tagName!="FIELDSET")
								{	this.parent().parent().parent().wrap("<fieldset class='ui-dform-fieldset-"+type+" ui-dform-fieldset' id='fset_"+that[0]['id']+"'></fieldset>");
								}
								/*END - FIELDSET AROUND GROUP*/

								var multi_has_req=-1;

								try
								{	surround=this.parent().parent()[0].outerHTML;
									multi_has_req=surround.indexOf("required");						
								}
								catch(err) {
								}
								
								if(multi_has_req>-1) 
								{	if(this.parent().parent().prev().hasClass("ui-dform-label"))
									{	this.parent().parent().prev().addClass("dform-label-required");
									}
								}
								/*START - FIELDSET AROUND GROUP*/
								if(this.parent().parent().prev().hasClass("ui-dform-label"))
								{	this.parent().parent().prev().addClass("test");
									legend_content=this.parent().parent().prev().html();
									legend_for=this.parent().parent().prev().prop("for");
									//console.log(legend_content+" "+ legend_for);
									if(this.parent().parent().prev().hasClass("dform-label-required"))
									{	this.parent().parent().parent().parent().prepend("<legend id='legend-"+legend_for+"' class='dform-legend dform-legend-required'>"+legend_content+"</legend>");				
									}
									else
									{	this.parent().parent().parent().parent().prepend("<legend id='legend-"+legend_for+"' class='dform-legend'>"+legend_content+"</legend>");				
									}
									this.parent().parent().prev().remove();
								}	
								/*END - FIELDSET AROUND GROUP*/
								
/*jQuery(".ui-dform-fieldset-checkbox, .ui-dform-fieldset-radio").each(function(){
	legend_content=jQuery(this).find(".ui-dform-label:first-child").html();
	legend_for=jQuery(this).find(".ui-dform-label:first-child").prop("for");

	if(jQuery(this).find(".ui-dform-label:first-child").hasClass("dform-label-required"))
	{	jQuery(this).prepend("<legend id='legend-"+legend_for+"' class='dform-legend dform-legend-required'>"+legend_content+"</legend>");				
	}
	else
	{	jQuery(this).prepend("<legend id='legend-"+legend_for+"' class='dform-legend'>"+legend_content+"</legend>");				
	}
	jQuery(this).find(".ui-dform-label:first-child").remove();
							...
							{	var that=this.parent();
								...
								var lab='<span class="option-label">'+ops["html"]+'</span>'; 					
								
								myid=this[0]['id'];	
								if(this[0].outerHTML.indexOf("required")>-1)
								{	var input='<input type="'+type+'" class="ui-dform-'+type+'" type="'+type+'" name="'+myid+'" id="'+myid+'" '+input_required+'>';
								}
								else
								{	var input='<input type="'+type+'" class="ui-dform-'+type+'" type="'+type+'" name="'+myid+'" id="'+myid+'" '+'>';
								}
								if(this[0].outerHTML.indexOf("required")>-1)
								{	this.wrap("<label for='"+myid+"'class='"+label_type+" "+label_required_class+" control-label'> </label>" );				
								}
								else
								{	this.wrap("<label for='"+myid+"'class='"+label_type+" "+" control-label'> </label>" );				
								}
								this.parent().append($(label));
								this.parent().wrap("<div class='form-group'></div>");		
								var parent=this.parent();
								parent.empty();
								parent.append(input);
								parent.append(lab);	
					...
					else //// this handles singlular radiobuttons
					{	var lab=ops["html"]; 
						var that=this.parent()
						var this_attributes=new Array();
						...
						{	this.wrap("<label class='"+type+" "+label_required_class+" control-label'> </label>" );				
						}
						else
						{	this.wrap("<label class='"+type+" "+" control-label'> </label>" );				
						}
		...
		"[post]" : function (options, type) {
			/** original statement (commented below) replaced by Alan wth two statements 
			  * to add a [] to checkboxes group so that php will look for an array of results
	...
	$.dform.subscribe("[pre]",
		...
		{//console.log(options);
			if(type == "form")
			{
				...
				{	defaults = {
						highlight: function(input)
						{	$(input).closest(".form-group").removeClass("val-good");
							$(input).closest(".form-group").addClass("val-error");
						},
						unhighlight: function(input)
						{
							$(input).closest(".form-group").removeClass("val-error");
							$(input).closest(".form-group").addClass("val-good");
						}
					};
				}
